"""Infra Discovery publication and Unix status socket lifecycle."""
...
class InfraDiscoveryError(RuntimeError):
    """The local discovery or status publication boundary is unsafe."""
...
def _runtime_root() -> Path:
    ...
    if sys.platform == "darwin":
        ...
        # Darwin _CS_DARWIN_USER_TEMP_DIR is 65537.
        length = int(confstr(65537, None, 0))
        if length <= 1:
...
def _socket_capacity() -> int:
    return 104 if sys.platform == "darwin" else 108


def _validate_socket_path(path: Path) -> None:
    required = len(os.fsencode(str(path))) + 1
    if required > _socket_capacity():
        raise InfraDiscoveryError(
            f"Unix socket path requires {required} bytes; maximum is {_socket_capacity()}"
        )
...
class DiscoveryRuntime:
    ...
    def resolve_socket(self, endpoint: str) -> Path:
        ...
        if (
            ...
            or any(not (character.isascii() and (character.isalnum() or character in "._-")) for character in opaque)
        ):
            raise InfraDiscoveryError("Unix socket endpoint is invalid")
...
class _PublicationAuthority:
    def __init__(self, path: Path) -> None:
        ...
        try:
            ...
            os.close(descriptor)
            raise InfraDiscoveryError(
                "cannot secure Observer publication authority lock"
            ) from error
        try:
...
class ObserverFacilityService:
    ...
    def __init__(
        ...
        *,
        ...
        self.manifest_path = self.runtime.registrations / f"{SERVICE_KIND}--{SERVICE_INSTANCE_ID}.json"
    ...
    def _select_endpoint(self) -> None:
        opaque = f"dm-{uuid.uuid4().hex[:12]}"
        self.endpoint = f"sockets/{opaque}.sock"
    ...
    def _bind_listener(self) -> socket.socket:
        ...
        for attempt in range(ENDPOINT_BIND_ATTEMPTS):
            ...
            return listener
        raise InfraDiscoveryError(
            "cannot allocate a unique Observer Unix socket endpoint"
        ) from last_collision
    ...
    def start(self) -> None:
        ...
        if self._started_once:
            ...
            self._select_endpoint()
        authority_path = self.runtime.registrations / f".{SERVICE_KIND}--{SERVICE_INSTANCE_ID}.publisher.lock"
    ...
    def _handle_connection(self, connection: socket.socket) -> None:
        try:
            ...
            payload = json.dumps(response, ensure_ascii=False, separators=(",", ":")).encode("utf-8") + b"\n"
